...
class admin_point_model extends CI_Model {
    ...
    public function getPointCount($search) {
        ...
        $sql = "select id from db_points where status = 0" . $where;
    ...
    public function getPointList($search) {
        ...
        $sql = "select a.*,b.name as typename from db_points as a left join db_point_type as b on a.type_id=b.id where a.status = 0 " . $where . " order by a.id desc limit " . $search['start'] . ',' . $search['pagesize'];
        $query = $this->db->query($sql);
        return $query->result();
    }
     ...
     *
     */
    public function savePointData($data) {
        $this->db->insert('db_points', $data);
     ...
     *
     */
    public function editPointData($data, $id) {
     ...
     * 
     */
    public function delPointById($data,$id) {
        return $row = $this->db->update('db_points', $data, array('id' => $id));
    }
}