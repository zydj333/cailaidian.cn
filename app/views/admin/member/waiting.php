...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <script type="text/javascript">
            function check(id) {
                $.ajax({
                    url: '/admin_member_waiting/check/' + id + '/' + Math.random(),
                    success: function() {
                        location.href = "/admin_member_waiting/index";
                    },
                    cache: false
                });
            }
            ...
            function getPageUrl(nowpage) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/admin_member_waiting/ajaxList/" + Math.random(),
                    ...
                    success: function(data) {
                        ...
                        if (data.flag == 0) {
                            ...
                            $.each(data.error, function(key, values) {
                                ...
                                str += '</td>';
                                str += '<td>';
                                str += '<a onclick="return check(' + values.id + ')" href="javascript:void(0);" class="tablelink">通过审核</a>';
                                str += '</td>';
                                str += '</tr>';
                            });
                        }
    ...
    <body>
        <div class="place">
            ...
            <ul class="placeul">
                ...
                <li><a href="/admin_member_waiting/index">待审核用户</a></li>
            </ul>
        </div>
        <div class="rightinfo">
            <ul class="seachform">
                    ...
                    <?php if (!empty($list)): ?>
                        <?php foreach ($list as $key => $value): ?>
                            <tr>
                                ...
                                <td><?php if ($value->ischeck == 0): ?>否<?php else: ?><span style="color:red;">已删除</span><?php endif; ?></td>
                                <td><a onclick="return check(<?php echo $value->id; ?>)" href="javascript:void(0);" class="tablelink">通过审核</a></td>
                            </tr> 
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
