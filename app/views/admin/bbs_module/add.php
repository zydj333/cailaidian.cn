<div class="formbody">
    ...
    <form action="#" method="post" id='bbs_module_add_form'>
        <ul class="forminfo">
            <li><label>分类标题</label><input name="cate_name" type="text" class="dfinput" maxlength="20" /><i>请填写模块的标题（长度不超过20个字符）</i></li>
            <li>
                <label>父级名称</label>
                <div class="vocation">
            ...
            <li>
                ...
                <div class="vocation">
                    ...
                    <input type="hidden" name="cate_image_url" id="cate_image_url" value="" />
                </div>
            </li>
            <li></li>
            <li><label>模块描述</label><textarea class="textinput" name="cate_description" cols="150" rows="3"></textarea><i>请先填写此模块的一些说明</i></li>
            <li><label>排序</label><input name="cate_salt" type="number" class="dfinput" maxlength="10" value="0" /><i>填写一个数字作为排序（只能为整数）</i></li>
            <li><label>&nbsp;</label><input id='bbs_module_add_button' type="button" class="btn" value="确认保存"/></li>
        </ul>
    </form>
</div>
...
<script type="text/javascript">
    $(document).ready(function(e) {
        ...
        $("#bbs_module_add_button").click(function() {
            $.ajax({
                type: "POST",
                url: "/admin_bbs_module/add/" + Math.random(),
                data: $("#bbs_module_add_form").serialize(),
                dataType: "json",
